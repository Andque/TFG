...
H=feedback(Z_p,B_p);
Tu_par=series(H,A_p);
Tu_par_ret=series(Tu_par,G_delay);
h=bodeplot(Tu_par_ret,'g--');
p.FreqUnits = 'Hz';
setoptions(h,p);
xlim([0 10^5])
ylim([-180 370])
...
figure('Name','Con retardo vs sin retardo en promediado')
hold on
h=bodeplot(Tu_par,'r');
p.FreqUnits = 'Hz';
setoptions(h,p);
xlim([0 10^5])
ylim([-180 370])
h=bodeplot(Tu_par_ret,'b');
p.FreqUnits = 'Hz';
setoptions(h,p);
xlim([0 10^5])
ylim([-180 370])

figure('Name','Con parasitas vs sin parasitas con retardo en promediado')
hold on
h=bodeplot(Tu_par_ret,'r');
p.FreqUnits = 'Hz';
setoptions(h,p);
xlim([0 10^5])
ylim([-180 370])
...
figure('Name','SS vs promediado con retardo ')
hold on
h=bodeplot(Tu_par_ret,'r');
p.FreqUnits = 'Hz';
setoptions(h,p);
...
xlim([0 10^5])
ylim([-180 370])
figure('Name','SS vs promediado sin retardo ')
hold on
h=bodeplot(Tu_par,'r');
p.FreqUnits = 'Hz';
setoptions(h,p);
