...
Tvuz=series(Tvuz,H);
bode(Tvuz)
%Ver cual es la ganancia en DC
[mag1,phase1,wout1] = bode(Tvuz,0.1);

if(mag1*q_pwm<q_adc*alfa)
    fprintf("Se cumple A1, por lo cual puede que no haya ciclos limites");
else
    fprintf("No se cumple A1, por lo cual puede que haya ciclos limites");
end
